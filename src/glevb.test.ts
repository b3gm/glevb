...
import { ux2 } from '@b3gm/algebr4';
import { Vec2 } from '@b3gm/algebr4';

const glevb:Glevb = new Glevb({
...
function checkProjectiles(now:number) {
  const kickTime: number = now - Projectile.lifeTime;
  let i = 0;
...
function fireProjectile(ship:Ship, now:number) {
  ...
  const pos:Vec2 = ship.position.copy();
  const s: Vec2 = ship.speed.add(
    ux2.rotateRad(ship.orientation)
      .scalarMultiply(2.5)
  );
...
class Planet {
  ...
  public render(ctx:CanvasRenderingContext2D, shipPosition:Vec2) {
    const diffVec:Vec2 = this.pos.subtract(shipPosition);
    const distance:number = diffVec.norm();
    if(distance > this.radius + 480) {
      // RenderUI
      ctx.translate(shipPosition.x, shipPosition.y);
      ctx.beginPath();
      const eDiff:Vec2 = diffVec.normalize();
      const begin: Vec2 = eDiff.scalarMultiply(40);
      const end:Vec2 = eDiff.scalarMultiply(60);
      const tPos:Vec2 = eDiff.scalarMultiply(80);
...
class Ship {
  ...
  public propagate(af:number, trF:Vec2) {
    this.lastForce = trF;
    const rotatedForce = trF.rotateRad(this.orientation);
    this.speed.x += rotatedForce.x;
    this.speed.y += rotatedForce.y;
    this.position = this.position.add(this.speed);
...
const canvas:HTMLCanvasElement = <HTMLCanvasElement>document.getElementById('cvsViewPort');
...
function vecProd(a:Vec2, b:Vec2) {
  ...
  return ua.x * ub.y - ub.x * ua.y;
}
function sigAngle(a:Vec2, b:Vec2) {
  return Math.asin(vecProd(a, b));
}

const forward:Keys = Keys.KeyW;
const backward:Keys = Keys.KeyS;
const strafeLeft:Keys = Keys.KeyA;
const strafeRight:Keys = Keys.KeyD;
const fireButton:Keys = Keys.Mouse1;
let lastSFX = 0;
let lastSFY = 0;
...
function gameLoop() {
  ...
  const cx: number = cursor.position.x + ev.mouseProperties.movement.x;
  const cy: number = cursor.position.y + ev.mouseProperties.movement.y;
  ...
  const cpos:Vec2 = cursor.position.add(ship.position);
  cpos.x -= 320;
  cpos.y -= 240;

  const diffVec:Vec2 = cpos.subtract(ship.position);

  let af:number;
  if(diffVec.norm() > 10) {
    const shipVec:Vec2 = ux2.rotateRad(ship.orientation);

    const angleDiff: number = sigAngle(shipVec, diffVec);
  ...
  const cF:Vec2 = new Vec2(0, 0);
  const mod:number = interval / 33;
  ...
  const sf0 = getStarFieldZero(ship.position);
...
class Ball {
  ...
  public update(ctx:CanvasRenderingContext2D) {
    ++this.lifeTime;
    const r = this.r = this.r + 1 / 100;
    this.squash = this.r * 0.3;
    this.squashFrequency = 10 / this.r * 0.5;
    ++this.squashTime;
    const s = this.squash * Math.exp(-this.squashTime * this.squashDecay);
    const trig = Math.cos(this.squashTime * this.squashFrequency);
    const rx = Math.max(0, r + trig * s);
    const ry = Math.max(0, r - trig * s);
...
class MouseBall {
  ...
  public update(ctx:CanvasRenderingContext2D, position:Vec2) {
    ...
    const mag:number = mVec.norm();
    let angle:number = ux2.angle(mVec);
    if (mVec.y < 0) {
      angle *= -1;
    }
...
function createBall() {
  const p:Vec2 = new Vec2(Math.random() * 640, Math.random() * 480);
  const v:Vec2 = new Vec2(Math.random() * 10 - 5, Math.random() * 10 - 5);
  return new Ball(p, v, 'hsl(' + Math.floor(Math.random() * 360) + ', 100%, 50%)');
}

document.addEventListener('DOMContentLoaded', () => {
  const cvs:HTMLCanvasElement = <HTMLCanvasElement>document.getElementById('cvsUnlocked');
  ...
  const sys2 = new Glevb({
    interval: 1000 / 30,
    runLoop: true,
    root: cvs
  });
  const mouseBall:MouseBall = new MouseBall(50);
  sys2.onLoop((ev) => {
