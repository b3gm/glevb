...
export default class Glevb {
  ...
  private _scrollHandler:(ev:WheelEvent) => void;
  ...
  constructor(settings:GlevbSettings) {
    ...
    this._scrollHandler = (ev:WheelEvent) => {this.scrollHandler(ev);};
  ...
  private scrollHandler(ev:WheelEvent) {
  ...
  public watchKeys(...keys:Array<Keys>) {
    ...
    for(const k of keys) {
      ...
      if(!strKey) {
        this.errorLog('Unknown keycode', k);
        continue;
      }
